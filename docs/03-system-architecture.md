...
| 문서 상태 | `review` |
| 마지막 검토일 | 2026-07-23 |
| 선행 문서 | [프로젝트 개요](./00-project-overview.md), [범위와 목표](./01-scope-and-goals.md), [개발 규칙](./02-development-rules.md), [ADR-001](./decisions/ADR-001-initial-model-scope.md) |
| 후속 문서 | [모델 아키텍처](./04-model-architecture.md), [토크나이저 설계](./05-tokenizer-design.md), [사전학습 계획](./08-pretraining-plan.md), [SFT 계획](./09-sft-plan.md), [저장소 구조](./21-repository-structure.md), [산출물·설정 정책](./22-artifact-and-configuration-policy.md) |
| 구현 전 필수 여부 | 예 |
...
- [확정] 상세 산식과 tensor shape는 [모델 아키텍처](./04-model-architecture.md)를 단일 기준으로 삼는다.
...
상세 정책은 [산출물·설정 정책](./22-artifact-and-configuration-policy.md)을 따른다.
