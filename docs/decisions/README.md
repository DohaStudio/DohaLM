...
| ADR-032 | [Production Training Intent Authority](./ADR-032-production-training-intent-authority.md) | `draft` | 미결정 | [ADR-021](./ADR-021-production-training-adapters-and-durable-journal.md), [C3 PostgreSQL Training Composition](../architecture/c3-postgresql-training-composition.md) | dedicated submitter authority, immutable intent/idempotency, validated request projection과 append-only decision binding 제안 | ADR 승인 전 foundation 구현·application entrypoint·Training activation 금지 |
...
- [제안] ADR-032는 dedicated local submitter authority, 별도 intent/run identity, immutable submission, submitter-scoped idempotency와 validated request projection 뒤 append-only decision binding을 제안한다. ADR 승인 전 foundation 구현과 production Training activation은 금지한다.
