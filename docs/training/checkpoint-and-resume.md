...
| 후속 문서·작업 | Gate 6 검토, 운영 checkpoint schema·retention 결정 |
...
| `training-state.json` | 진행 state와 Python·torch CPU·가능 시 CUDA RNG |
...
6. RNG 복원
7. 결정론적 DataLoader를 `micro_step`까지 fast-forward
...
- [검증 필요] 운영 dataset sampler state와 대규모 DataLoader의 빠른 위치 복원 방식은 Phase 5 확장 시 별도 구현한다.
