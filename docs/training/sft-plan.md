...
- [확정] special token 문자열과 ID는 [토크나이저 설계](./tokenizer-design.md)를 따른다.
...
- [검증 필요] 자동·사람 평가의 정량 합격선은 [평가 계획](../evaluation/evaluation-plan.md)에 따라 확정한다.

## 11. SFT 체크포인트 추가 항목

[사전학습 계획](./pretraining-plan.md)의 공통 checkpoint 항목에 다음을 추가한다.
