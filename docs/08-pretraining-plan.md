...
- [확정] 모델은 [모델 아키텍처](./04-model-architecture.md), 토큰은 [토크나이저 설계](./05-tokenizer-design.md)를 따른다.
...
세부 조정 순서는 [GPU 메모리 전략](./16-gpu-memory-strategy.md)을 따른다.
...
- [검증 필요] 데이터 규모, token budget 및 train/validation 비율
