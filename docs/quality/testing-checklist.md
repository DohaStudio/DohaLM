...
| TRN-005 | Pilot | local-only corpus·16k tokenizer·split/packing·validation·checkpoint/resume·generation | Unit/Integration/GPU | 예 | Stage A 회귀 통과, Stage B 100-step 이하 증거 | 실제 실행 중단·계보와 권리·OOM 원인 재검토 | 예/일부 | `partial` — canonical pilot-v2 5-step Runtime Smoke·checkpoint load-only·mismatch 검증과 581개 회귀 통과; 100-step 최종 실행은 미승인 |
...
- [확정] canonical pilot-v2 전용 5-step Runtime Smoke에서 finite loss·AMP·VRAM·evaluation·checkpoint checksum·load-only resume·mismatch 차단을 확인했고 전체 581개 테스트가 통과했다. 100-step Pilot은 미승인이다.
