...
- [확정] 2026-09-01 기준 manifest는 63개 항목이며 repository 후보 62/62를 분류한다. required는 30파일, non-required는 33파일이며 이 중 7파일은 `delegated`다. ADR-032 foundation/application과 ADR-033 eligibility/config/accountability tests는 Training required에서 실행하고, C1은 migration/static/restore를 포함한 23 tests, C2는 shared PostgreSQL fixture와 authority identity separation을 소유한다.
- [확정] Training canonical required suite는 ADR-033의 production scope·eligibility 7 tests와 authority separation 3 tests를 추가한 492 tests다. C2 canonical은 authority separation 3 tests를 추가한 262 tests이며 Dataset canonical 384 tests와 C1 canonical 23 tests는 변하지 않는다. 위임된 경로는 validator가 owner workflow의 exact context, pytest target, classifier에 모두 존재하는지 정적으로 검증하며 live ruleset binding은 policy Gate에서 별도로 검증한다. 직전 503/503, 432/432, 460/460과 482/482 baseline은 historical evidence로 보존한다.
