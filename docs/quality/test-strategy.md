...
- [확정] 2026-09-02 기준 manifest는 66개 항목이며 repository 후보 64/64를 분류한다. required는 33파일, non-required는 33파일이며 이 중 7파일은 `delegated`다. ADR-032/033 foundation과 Production authority provisioning/application tests는 Training required에서 실행한다. C1은 migration/static/restore와 `0001–0008 → 0009` upgrade를 포함한 30 tests, C2는 shared PostgreSQL fixture·authority adapter·identity separation을 소유한다.
- [확정] Dataset pair C3 payload compatibility remediation 반영 후 canonical required suite는 Dataset 434, C1 31, C2/C3/Host 271, Training 520 tests다. migration 0010은 v1 pair authority payload와 DatasetVersion·DatasetManifest·canonical content-pair fingerprint를 보존하고, authoritative `upstream_objects`, `artifact_references`, `evaluated_at`, `expected_split_id`를 포함한 v2 payload를 새 payload fingerprint/authority identity로 발행한 뒤 append-only supersession event로 unique current pair를 전환한다. exact replay는 같은 v2 identity를 반환하고 conflicting payload, stale prerequisite, split/artifact mismatch와 비인가 mutation은 fail closed한다. frozen C3 resolver·Host·backend 의미는 변경하지 않는다. 이전 Dataset 432·425·386, C1 30·26, C2 269·267, Training 519·518·504와 503/503, 432/432, 460/460, 482/482, 492/492 baseline은 historical evidence로 보존한다.
...
| 날짜 | 변경 내용 |
|---|---|
| 2026-09-02 | [확정] Dataset pair authority v2 payload completeness·append-only supersession·C3 no-fallback 회귀를 required owner에 편입하고 Dataset 434·C1 31·C2 271·Training 520 canonical count 반영 |
