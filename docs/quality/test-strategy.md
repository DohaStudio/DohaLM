...
- [확정] ADR-036 indefinite-while-current retention compatibility와 ADR-037 large Product Dataset proposal authority 반영 후 canonical required suite는 Dataset 432, C1 30, C2 269, Training 519 tests다. Dataset은 owner CurrentEvidence가 exact Common projection과 SourceToken currentness를 재검증할 때만 boolean indefinite retention을 허용하며, finite retention은 기존 expiry-bound 검증을 유지한다. revoked·superseded·stale·identity/fingerprint mismatch는 fail closed하고 synthetic expiry를 만들지 않는다. 또한 manifest-reference v2의 bounded root·replay·tamper·million-member metadata와 review·approval·publication 재현 계약을 포함한다. C1은 migration 0009의 v1 보존·v2 create/read/replay/conflict 계약을 포함한다. 위임된 경로는 validator가 owner workflow의 exact context, pytest target, classifier에 모두 존재하는지 정적으로 검증하며 live ruleset binding은 policy Gate에서 별도로 검증한다. 이전 Dataset 425·386, C1 26, C2 267, Training 518·504와 503/503, 432/432, 460/460, 482/482, 492/492 baseline은 historical evidence로 보존한다.
