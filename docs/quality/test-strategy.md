...
- [확정] Training canonical required suite는 기존 283, critical 211, guard 9를 합친 503 tests다. 로컬 재시도 없는 실행은 503/503 pass였고 critical 18파일은 211/211 pass, skip·deselect 0이었다. test-only 합산 시간은 약 117.56초로 기존 대표값 약 80초 대비 37.56초(46.9%) 증가해 runtime 영향은 `HIGH`로 분류한다. clean GitHub Actions end-to-end 시간은 PR run evidence로 별도 기록한다.
