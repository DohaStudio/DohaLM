...
- [확정] 중복 설명 대신 [프로젝트 개요](../project/overview.md), [범위와 목표](../project/scope-and-goals.md) 및 세부 기준 문서를 링크한다.
...
| FastAPI | 로컬 추론 API | [확정] Base Qwen Chat·SSE 구현 |
| Next.js | 로컬 채팅 화면 | [확정] Base Qwen 연동 구현 |
| Docker | 배포 환경 | [제외] 현재 범위 밖 |

- [확정] 새 의존성은 필요성, 라이선스, 버전 호환성 및 대체 가능성을 확인한 뒤 추가한다.
- [확정] Phase 0의 Python 지원 범위는 `>=3.10,<3.13`이며 현재 Python 3.12.5에서 검증했다.
- [확정] PyYAML은 runtime, pytest는 development, PyTorch와 NumPy는 optional GPU 의존성으로 분리한다.
...
- [검증 필요] 정확한 로그 형식, 실험 ID 및 산출물 디렉터리 규칙은 [실험 관리 정책](../training/experiment-management.md)에 따라 구현 전에 확정한다.
...
다음 값은 [모델 아키텍처](../architecture/model-architecture.md)와 [ADR-002](../decisions/ADR-002-tiny-model-architecture.md)를 기준으로 관리한다.
...
- [검증 필요] 세부 승인 기준은 [데이터 전략](../data/data-strategy.md)과 [데이터 전처리](../data/preprocessing.md)에 따라 구현 전에 확정한다.

## 8. Git 브랜치 및 커밋 규칙

- [확정] `main`은 검증된 변경만 유지하고 기능 개발은 목적별 브랜치에서 수행한다.
- [확정] 브랜치명은 `docs/`, `feat/`, `fix/`, `test/`, `experiment/` 접두어와 짧은 목적을 조합한다.
- [확정] 사용자가 현재 요청에서 커밋을 명시적으로 요청한 경우에만 커밋한다.
- [확정] 커밋 하나에는 하나의 논리적 변경만 담는다.
...
- [확정] 사용자 요청 없는 commit·push를 금지하며 merge·rebase·tag·release도 사용자의 현재 요청이 있어야 한다.
...
- [검증 필요] 통합 테스트의 구체적 합격 기준은 [테스트 체크리스트](../quality/testing-checklist.md)에 따라 구현 전에 확정한다.
...
| 날짜 | 변경 내용 |
|---|---|
| 2026-08-04 | FastAPI·Next.js 로컬 구현과 Docker 배포 제외 범위 반영 |
