...
- [확정] Phase 0 설정은 YAML, 필수 필드·타입·알 수 없는 필드 거부, Tiny 불변 조건 검사를 적용한다.
- [확정] 병합 순서는 버전 관리 모델 설정 → 실행 목적 설정 → 명시적 CLI override이며, 최종 resolved config는 비밀값을 마스킹한다.
- [검증 필요] 학습용 미결정값과 `DohaLM-Small` 상세 구조는 실행 가능한 기본값으로 추정하지 않는다.
...
1. [확정] 명시적 명령행 인자
2. [확정] 실행 목적 설정
3. [확정] 버전 관리되는 모델 기본 설정 파일

- [확정] Phase 0 설정 로더는 위 우선순위와 알 수 없는 override의 오류 중단을 구현한다.
- [확정] 비밀값은 위 일반 설정 병합 체계와 분리해 환경 변수 또는 비밀 관리 수단으로만 공급한다.
- [확정] 우선순위 적용 후의 최종값을 실행 기록에 남겨야 하며 입력 파일만 기록해서는 안 된다.
- [확정] 정의되지 않은 키, 형식 오류, 호환되지 않는 override는 오류로 중단한다.
...
- [확정] DohaLM-Tiny의 기준 사양과 예상 파라미터 16,889,856은 [모델 아키텍처](../architecture/model-architecture.md)와 [ADR-002](../decisions/ADR-002-tiny-model-architecture.md)를 따른다.
