...
| 문서 상태 | `review` |
| 마지막 검토일 | 2026-07-23 |
| 선행 문서 | [개발 규칙](./02-development-rules.md), [개발 로드맵](./17-development-roadmap.md), [Definition of Ready](./31-definition-of-ready.md), [Definition of Done](./32-definition-of-done.md), [ADR-006](./decisions/ADR-006-development-quality-gates.md) |
| 후속 문서 | [테스트 체크리스트](./18-testing-checklist.md), 실제 test 구현 [검증 필요] |
| 구현 전 필수 여부 | 예 |

- [확정] 현재 테스트 코드는 구현하지 않았으며 이 문서는 수준·환경·데이터·실패 처리 기준이다.
...
- [검증 필요] 실제 명령, timeout, CI matrix, coverage와 합격 시간은 구현 후 결정한다.
- [제외] 이번 단계에서는 CI workflow를 생성하지 않는다.
