...
- [확정] Phase 0·1 기반, 운영 `operating-16k-v2/unigram-16k`, DohaLM-Tiny, Trainer, 실제 corpus Overfit, canonical Pilot·Candidate A 실행 backend와 Evaluation Framework는 구현·검증 또는 실행됐다. 서비스·Frontend·SFT와 Candidate B training은 미구현 또는 미실행이다.
...
아래 트리는 2026-07-28 현재 주요 구조를 요약한다. `.git/`과 대부분의 파일 단위 세부 목록은 생략했다.
...
│       ├── foundation-model-strategy.md
│       ├── model-family-roadmap.md
│       ├── model-lineage.md
│       └── domain-model-strategy.md
├── frontend/
├── scripts/
├── server/
├── src/
...
| `src/training/` | Trainer와 fail-closed pretraining backend | config·dataset/collator/loader·stateful sampler·AdamW·scheduler·AMP·accumulation·metric·memory/throughput·checkpoint/resume·Pilot/Candidate orchestration | 실제 corpus·checkpoint·대용량 실행 결과, SFT, 운영 실험 registry | 소스만 추적 | Gate 6·7과 Pilot·Candidate A 검증 완료 | 각 실제 실행의 별도 승인 전 |
...
| 날짜 | 변경 내용 |
|---|---|
| 2026-07-28 | [확정] 현재 구현·실행 상태와 장기 Foundation·Model Family 문서 구조를 반영함 |
