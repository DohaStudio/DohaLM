...
- [확정] `src/config/`, `src/runtime/`, `src/cli/`, Phase 1 `src/data/`, `scripts/datasets/` 읽기 전용 분석기와 관련 테스트는 구현됐다. 그 밖의 모델·학습·서비스 기능은 스캐폴드이며 `frontend/`에는 안내 문서만 있다.
...
| `scripts/` | 반복 작업 진입점 | 준비·학습·실행 스크립트, 외부 데이터 읽기 전용 구조 분석기 | 실제 원본·개인 절대 경로, 핵심 학습 모델 | 추적 | 필요 시 검토 | 해당 작업 구현과 함께 |
