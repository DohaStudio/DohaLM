...
| 문서 상태 | `review` |
| 마지막 검토일 | 2026-07-23 |
| 선행 문서 | [범위와 목표](./01-scope-and-goals.md), [개발 규칙](./02-development-rules.md), [시스템 아키텍처](./03-system-architecture.md), [평가 계획](./10-evaluation-plan.md), [실험 관리](./15-experiment-management.md), [ADR-006](./decisions/ADR-006-development-quality-gates.md) |
| 후속 문서 | [테스트 체크리스트](./18-testing-checklist.md), [Definition of Ready](./31-definition-of-ready.md), [Definition of Done](./32-definition-of-done.md), [테스트 전략](./33-test-strategy.md), [위험 등록부](./34-risk-register.md), [버전 계획](./35-version-plan.md), [Codex 작업 절차](./36-codex-workflow.md) |
| 구현 전 필수 여부 | 예 |
