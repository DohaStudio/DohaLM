...
현재 저장소는 Gate 1~6을 통과했습니다. Phase 1 DATA-001~016, 운영 16k v2 Unigram, Phase 3·4 모델, Phase 5 Trainer Foundation과 실제 Tiny 규모 CUDA 학습 검증은 구현·테스트됐습니다. Gate 7은 `planned`를 유지합니다.
...
| 학습 기반 | Trainer와 실제 Tiny 규모 sampler·cosine·FP16 AMP·checkpoint/resume·VRAM 측정, 제한 실제 corpus 500-step 검증 완료, Gate 6 `passed`, Gate 7 `planned` |
| 데이터 | AIHUB-71748 학생·비상업 tokenizer development와 제한 Tiny Overfit만 승인, Pretraining·PII·재배포 미승인 |
...
Gate 4·5·6은 evidence bundle과 514개 테스트를 근거로 2026-07-24 사용자 승인되어 `passed`이고, Gate 3은 2026-07-26 승인된 v2 Unigram으로 `passed`입니다. 제한 실제 corpus Tiny Overfit은 loss 감소와 resume를 확인했지만 exact continuation에 실패해 Gate 7과 Pilot Pretraining readiness는 계속 `planned`·`blocked`입니다.
