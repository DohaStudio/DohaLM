...
개발은 저장소·환경 기반부터 데이터, 토크나이저, 모델, 학습, 평가, SFT, 서비스 순서로 진행합니다. 각 단계는 문서·구현·테스트 증거를 요구하는 Gate로 관리하며, Gate 0 문서 승인은 사용자에 의해 `approved`로 확정되었습니다. 이는 구현 완료를 의미하지 않으며 현재 Phase 0 저장소·환경 기반 구현을 시작할 수 있는 상태입니다.
