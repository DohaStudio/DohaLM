...
개발은 저장소·환경 기반부터 데이터, 토크나이저, 모델, 학습, 평가, SFT, 서비스 순서로 진행합니다. 각 단계는 문서·구현·테스트 증거를 요구하는 Gate로 관리합니다. Gate 0 문서 승인은 `approved`, Gate 1 환경 검증은 `passed`로 확정되었으며 Phase 1 데이터 최소 파이프라인에 진입할 수 있습니다. 이는 모델·학습·서비스 구현 완료를 의미하지 않습니다.
