...
CURATED_PACKED_SEQUENCE_COUNT = 278_535
ONE_EPOCH_OPTIMIZER_STEPS = 34_817
ONE_EPOCH_INPUT_TOKENS = 71_304_960
...
class FullPretrainingConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if any(
            ...
            raise TrainingError(
                ...
                "token·step·batch 값은 양의 정수여야 합니다.",
            )
        if (
            self.tokens_per_optimizer_step
            != self.effective_batch_size * self.model.context_length
        ):
            raise TrainingError(
                "FULL_PRETRAINING_TOKEN_BUDGET_MISMATCH",
                "step당 token 수가 batch×context와 일치하지 않습니다.",
            )
        expected_steps = math.ceil(self.token_budget / self.tokens_per_optimizer_step)
        if self.max_steps != expected_steps:
            raise TrainingError(
                "FULL_PRETRAINING_STEP_BUDGET_MISMATCH",
                "max_steps가 token budget 상한과 일치하지 않습니다.",
            )
        if (
            self.token_budget > MAXIMUM_PLANNING_TOKENS
            or self.maximum_epochs > MAXIMUM_PLANNING_EPOCHS
        ):
            raise TrainingError(
                "FULL_PRETRAINING_BUDGET_LIMIT",
                "준비 패키지의 최대 비교 범위는 3 epoch입니다.",
            )
        scheduled_epoch = self.scheduled_tokens / TRAIN_TOKEN_COUNT
        if self.maximum_epochs + 1e-9 < scheduled_epoch:
            raise TrainingError(
                "FULL_PRETRAINING_EPOCH_LIMIT",
                "maximum_epochs가 scheduled token 범위보다 작습니다.",
            )
        if self.scheduler_type != "cosine" or not 0 <= self.min_lr_ratio <= 1:
            raise TrainingError(
                "INVALID_FULL_PRETRAINING_CONFIG",
                "cosine scheduler와 유효한 min_lr_ratio가 필요합니다.",
            )
        if self.warmup_steps > self.max_steps:
            raise TrainingError(
                "INVALID_FULL_PRETRAINING_CONFIG",
                "warmup_steps는 max_steps 이하여야 합니다.",
            )
        for name in (
            "initialization",
            "evaluation_policy",
    ...
    def _validate_candidate_a_profile(self) -> None:
        if self.resume_checkpoint is not None:
            if self.resume_approval_status != "approved_full_run_resume":
                raise TrainingError(
                    "FULL_PRETRAINING_RESUME_NOT_APPROVED",
                    "Full Run resume requires separate user approval.",
                )
            self._validate_one_epoch_continuation_profile()
            return
        exact = {
        ...
        if (
            self.initialization.get("mode") != "fresh_seed_17"
            or self.initialization.get("pilot_checkpoint_used") is not False
        ):
            raise TrainingError(
        ...
        if (
            ...
            and self.resume_approval_status != "approved_full_run_resume"
        ):
            raise TrainingError(
                "FULL_PRETRAINING_RESUME_NOT_APPROVED",
                "Full Run resume requires separate user approval.",
    ...
    def _validate_one_epoch_continuation_profile(self) -> None:
        exact = {
            "budget_candidate": (
                self.budget_candidate,
                "candidate_a_one_epoch_continuation",
            ),
            "token_budget": (self.token_budget, ONE_EPOCH_INPUT_TOKENS),
            "tokens_per_optimizer_step": (self.tokens_per_optimizer_step, 2_048),
            "max_steps": (self.max_steps, ONE_EPOCH_OPTIMIZER_STEPS),
            "micro_batch_size": (self.micro_batch_size, 2),
            "gradient_accumulation_steps": (self.gradient_accumulation_steps, 4),
            "learning_rate": (self.learning_rate, 3e-4),
            "weight_decay": (self.weight_decay, 0.1),
            ...
            "resume_approval_status": (
                self.resume_approval_status,
                "approved_full_run_resume",
            ),
        }
        changed = [
            name for name, (actual, expected) in exact.items() if actual != expected
        ...
        if changed:
            raise TrainingError(
                "FULL_PRETRAINING_CONTINUATION_PROFILE_MISMATCH",
                f"One-epoch continuation profile mismatch: {changed}",
            )
        expected_continuation = {
            "source_run_id": "run-aihub-71748-local-v1-r3",
            "source_checkpoint": "checkpoint-4883",
            "source_step": 4_883,
            "source_manifest_sha256": "sha256:7faea6d169e76e15577ac87cd98630592f1047f26c329189adb3f027f3388a48",
            "target_cumulative_steps": ONE_EPOCH_OPTIMIZER_STEPS,
            "packed_sequence_count": CURATED_PACKED_SEQUENCE_COUNT,
            "actual_cumulative_input_tokens": ONE_EPOCH_INPUT_TOKENS,
            "scheduler_horizon_policy": "extend_cosine_global_step_no_warmup_restart",
        ...
        if self.continuation != expected_continuation:
            raise TrainingError(
                "FULL_PRETRAINING_CONTINUATION_PROFILE_MISMATCH",
                "The exact r3-to-r4 continuation contract is required.",
            )
        if (
            self.initialization.get("mode") != "resume_full_checkpoint"
            or self.initialization.get("source_run_id")
            != expected_continuation["source_run_id"]
            or self.initialization.get("source_step")
            != expected_continuation["source_step"]
            or self.initialization.get("pilot_checkpoint_used") is not False
        ):
            raise TrainingError(
                "FULL_PRETRAINING_INITIALIZATION_MISMATCH",
                "The continuation must initialize only from the approved r3 checkpoint.",
            )
    ...
    def from_yaml(cls, path: str | Path) -> "FullPretrainingConfig":
        try:
            ...
            raise TrainingError(
                "INVALID_FULL_PRETRAINING_CONFIG",
                "Full Pretraining config를 읽을 수 없습니다.",
            ) from exc
        if not isinstance(value, dict):
            raise TrainingError(
                "INVALID_FULL_PRETRAINING_CONFIG", "config root는 mapping이어야 합니다."
            )
        unknown = set(value) - set(cls.__dataclass_fields__)
        if unknown:
            raise TrainingError(
                "INVALID_FULL_PRETRAINING_CONFIG",
                f"알 수 없는 config field: {sorted(unknown)}",
            )
        model_value = value.get("model", {})
        if not isinstance(model_value, dict):
            raise TrainingError(
                "INVALID_FULL_PRETRAINING_CONFIG", "model은 mapping이어야 합니다."
            )
...
def estimate_training_budget(token_target: int) -> dict[str, int | float]:
    ...
    if (
        isinstance(token_target, bool)
        or not isinstance(token_target, int)
        or token_target <= 0
    ):
        raise TrainingError(
            "INVALID_FULL_PRETRAINING_BUDGET", "token target은 양의 정수여야 합니다."
        )
...
def resolve_full_pretraining_path(config: FullPretrainingConfig, value: str) -> Path:
    ...
    external_root, _ = resolve_local_paths(
        resolve_repository_path(config.local_dataset_config)
    )
    resolved = (external_root / value).resolve()
    if external_root not in resolved.parents:
        raise TrainingError(
            "FULL_PRETRAINING_PATH_INVALID", "경로가 configured external root 밖입니다."
        )
    return resolved
...
def probe_full_pretraining_output(config: FullPretrainingConfig) -> dict[str, Any]:
    ...
    if output.exists():
        raise TrainingError(
            "FULL_PRETRAINING_OUTPUT_EXISTS",
            "Full Pretraining output 경로가 이미 존재합니다.",
        )
    ...
    try:
        ...
        if (
            published.read_bytes() != payload
            or digest != f"sha256:{hashlib.sha256(payload).hexdigest()}"
        ):
            raise TrainingError(
                "FULL_PRETRAINING_OUTPUT_PROBE_FAILED",
                "output probe checksum이 일치하지 않습니다.",
            )
    finally:
...
def _load_manifest(path: Path) -> dict[str, Any]:
    try:
        ...
        raise TrainingError(
            "FULL_PRETRAINING_MANIFEST_INVALID", "approval manifest를 읽을 수 없습니다."
        ) from exc
    if not isinstance(value, dict) or value.get("schema_version") != "1.0":
        raise TrainingError(
            "FULL_PRETRAINING_MANIFEST_INVALID",
            "approval manifest schema가 유효하지 않습니다.",
        )
    return value
...
def inspect_full_pretraining_readiness(
    ...
    *,
    ...
    approval = (
        manifest.get("execution_approval", {})
        if isinstance(manifest.get("execution_approval"), dict)
        else {}
    )
    source = (
    ...
    storage = (
        manifest.get("storage", {}) if isinstance(manifest.get("storage"), dict) else {}
    )
    blockers: list[str] = []
    ...
    for key in (
        ...
        "tokenizer_model_checksum",
        "tokenizer_vocab_checksum",
    ):
        if (
            not isinstance(identity.get(key), str)
            or FINGERPRINT_PATTERN.fullmatch(identity[key]) is None
        ):
    ...
    expected_gates = {
        "0": "approved",
        **{str(value): "passed" for value in range(1, 8)},
    }
    if gates != expected_gates:
        blockers.append("FULL_PRETRAINING_GATES_NOT_SATISFIED")
    pilot = (
        manifest.get("pilot_evidence", {})
        if isinstance(manifest.get("pilot_evidence"), dict)
        else {}
    )
    if not (
    ...
    try:
        lineage = _lineage(
            config
        )  # Full config intentionally shares the canonical artifact path contract.
        comparisons = {
        ...
        if identity.get("vocabulary_size") != 16_000 or identity.get(
            "special_token_ids"
        ) != list(range(8)):
    ...
    if config.is_continuation:
        ...
        if not _approved(continuation):
            blockers.append("FULL_PRETRAINING_CONTINUATION_NOT_APPROVED")
        if any(
            continuation.get(key) != value for key, value in config.continuation.items()
        ):
            blockers.append("FULL_PRETRAINING_CONTINUATION_MISMATCH")
    ...
    initialization = (
        manifest.get("initialization", {})
        if isinstance(manifest.get("initialization"), dict)
        else {}
    )
    if any(
        initialization.get(key) != config.initialization.get(key)
        for key in ("mode", "seed", "pilot_checkpoint_used")
    ):
        blockers.append("FULL_PRETRAINING_INITIALIZATION_MISMATCH")
    if initialization.get("mode") == "pilot_checkpoint" and not isinstance(
        initialization.get("artifact_fingerprint"), str
    ):
        blockers.append("FULL_PRETRAINING_INITIALIZATION_ARTIFACT_MISSING")
    if (
        initialization.get("initialization_fingerprint")
        != config.initialization_fingerprint
    ):
        blockers.append("FULL_PRETRAINING_INITIALIZATION_FINGERPRINT_MISMATCH")

    training_config = (
        manifest.get("training_config", {})
        if isinstance(manifest.get("training_config"), dict)
        else {}
    )
    expected_training = {
        "optimizer": "AdamW",
    ...
    if any(
        training_config.get(key) != value for key, value in expected_training.items()
    ):
    ...
    for section_name, expected in policy_pairs.items():
        ...
        comparable = {
            key: value for key, value in expected.items() if key != "approval_status"
        }
    ...
    required_storage = (
        ...
        and int(storage.get("available_bytes", 0))
        >= int(config.disk_budget.get("minimum_free_bytes_before_start", 0))
    )
    ...
    if (
        ...
        or COMMIT_PATTERN.fullmatch(source["git_commit"]) is None
    ):
    ...
    environment = (
        ...
        if isinstance(manifest.get("environment"), dict)
        else {}
    )
    if any(
    ...
    backend = (
        ...
        if isinstance(manifest.get("execution_backend"), dict)
        else {}
    )
    ...
    if not final_approval_present:
        ...
        preflight = (
            approval.get("execution_preflight", {})
            if isinstance(approval.get("execution_preflight"), dict)
            else {}
        )
        required_preflight = (
            "windows_sleep_disabled",
            "no_restart_or_update_scheduled",
            "plugged_power",
            "adequate_cooling_and_ventilation",
            "nvidia_gpu_recognized",
            "cuda_available",
    ...
    if (
        approval.get("execution_started") is True
        or approval.get("execution_completed") is True
    ):
    ...
    return {
        ...
        if blockers == ["FULL_PRETRAINING_NOT_APPROVED"]
        else ("ready_for_execution" if not blockers else "blocked"),
        ...
        "execution_backend_implemented": manifest.get("execution_backend", {}).get(
            "status"
        )
