...
def _write_json(path: Path, value: Any, *, replace: bool = False) -> None:
    ...
    if path.exists() and not replace:
        raise TrainingError("FULL_PRETRAINING_ARTIFACT_EXISTS", f"Artifact already exists: {path.name}")
    try:
        ...
        raise TrainingError("FULL_PRETRAINING_ARTIFACT_WRITE_FAILED", f"Failed to publish {path.name}.") from exc
    finally:
...
def dry_run_full_pretraining(
    ...
    *,
    ...
    report = inspect_full_pretraining_readiness(config_path, manifest_path, probe_output=probe_output)
...
class FullSafetyMonitor:
    ...
    def __init__(self, config: FullPretrainingConfig, output_root: Path, *, started_at: float | None = None):
        self.config = config
    ...
    def observe(self, metric: TrainingMetric) -> None:
        if metric.global_step > FINAL_CHECKPOINT_STEP:
            raise TrainingError("FULL_PRETRAINING_STEP_LIMIT", "Optimizer step limit exceeded.")
        if metric.global_step * 2_048 > SCHEDULED_TOKEN_LIMIT:
            raise TrainingError("FULL_PRETRAINING_TOKEN_LIMIT", "Scheduled token limit exceeded.")
        if not math.isfinite(metric.loss) or not math.isfinite(metric.gradient_norm_before_clip):
            raise TrainingError("NON_FINITE_LOSS", "Non-finite training metric detected.")
        if time.perf_counter() - self.started_at >= int(self.config.wall_clock_budget["hard_stop_seconds"]):
            raise TrainingError("FULL_PRETRAINING_WALL_CLOCK_LIMIT", "Wall-clock hard stop reached.")
        if metric.peak_memory_reserved > int(self.config.system_safety["maximum_reserved_vram_bytes"]):
            raise TrainingError("FULL_PRETRAINING_VRAM_LIMIT", "Reserved VRAM limit exceeded.")
        if metric.cpu_working_set_bytes is None:
            raise TrainingError("FULL_PRETRAINING_CPU_MEMORY_UNAVAILABLE", "CPU working-set measurement is unavailable.")
        if metric.cpu_working_set_bytes > int(self.config.system_safety["maximum_cpu_working_set_bytes"]):
            raise TrainingError("FULL_PRETRAINING_CPU_MEMORY_LIMIT", "CPU working-set limit exceeded.")
        free = shutil.disk_usage(self.output_root).free
        if free < int(self.config.disk_budget["minimum_free_bytes_during_run"]):
            raise TrainingError("FULL_PRETRAINING_DISK_MINIMUM", "Runtime free disk is below 5 GiB.")
        if _directory_bytes(self.output_root) > int(self.config.disk_budget["run_budget_bytes"]):
            raise TrainingError("FULL_PRETRAINING_OUTPUT_BUDGET", "Run output exceeded 2 GiB.")
        ...
        if self.amp_skips >= int(self.config.system_safety["repeated_amp_skip_limit"]):
            raise TrainingError("FULL_PRETRAINING_AMP_SKIP_LIMIT", "AMP skipped three consecutive optimizer steps.")
        ...
        loss_is_spike = bool(loss_baseline and metric.loss >= loss_baseline * loss_limit)
        gradient_is_spike = bool(
            grad_baseline and metric.gradient_norm_before_clip >= grad_baseline * grad_limit
        )
        ...
        if self.loss_spikes >= int(self.config.system_safety["loss_spike_consecutive_steps"]):
            raise TrainingError("FULL_PRETRAINING_LOSS_SPIKE", "Loss exceeded the rolling threshold for ten steps.")
        if self.gradient_spikes >= int(self.config.system_safety["gradient_norm_spike_consecutive_steps"]):
            raise TrainingError("FULL_PRETRAINING_GRADIENT_SPIKE", "Gradient norm exceeded the rolling threshold for ten steps.")


class SingleUseApprovalConsumer:
    """Atomically consume approval only after optimizer step 1 succeeds."""

    def __init__(self, path: Path, *, run_id: str, manifest_path: Path, readiness_fingerprint: str):
        self.path = path
        self.run_id = run_id
...
def _checkpoint_manifest(output_root: Path) -> dict[str, Any]:
    checkpoints = sorted(path for path in output_root.glob("checkpoint-*") if path.is_dir())
    if tuple(path.name for path in checkpoints) != ALLOWED_CHECKPOINTS:
        raise TrainingError("FULL_PRETRAINING_CHECKPOINT_POLICY_MISMATCH", "Only mid and final checkpoints are allowed.")
    return {
        "schema_version": "1.0",
        "checkpoints": {
            path.name: {
...
def run_full_pretraining(
    ...
    if config.resume_checkpoint is not None:
        raise TrainingError("FULL_PRETRAINING_RESUME_NOT_APPROVED", "Fresh Candidate A execution cannot resume.")
    output_root = resolve_full_pretraining_path(config, config.output_dir)
    if output_root.exists():
        raise TrainingError("FULL_PRETRAINING_OUTPUT_EXISTS", "Existing run/output reuse is blocked.")
    if shutil.disk_usage(output_root.parent).free < int(config.disk_budget["minimum_free_bytes_before_start"]):
        raise TrainingError("FULL_PRETRAINING_DISK_BUDGET_NOT_SATISFIED", "Start free disk is below 10 GiB.")
    ...
    train_loader = create_dataloader(
        train_dataset, collator, training, shuffle=True, stateful=True,
        dataset_fingerprint=lineage["dataset_fingerprint"],
    )
    evaluation_loader = create_dataloader(evaluation_dataset, collator, training, shuffle=False)
    tokenizer_report = validate_operating_candidate(resolve_full_pretraining_path(config, config.tokenizer_model).parent)
    if tokenizer_report.get("tokenizer_fingerprint") != lineage["tokenizer_fingerprint"]:
        raise TrainingError("FULL_PRETRAINING_TOKENIZER_MISMATCH", "Operating tokenizer identity mismatch.")
    run_identity = {
    ...
    trainer = Trainer(
        ...
        dataset_metadata={"kind": "full-pretraining-candidate-a-v1", **lineage, **run_identity},
        metric_filename="full-training-metrics.jsonl",
    )
    approval = json.loads(json.dumps(readiness_report))
    _write_json(output_root / "full-execution-manifest.json", {
        "schema_version": "1.0", "status": "running", "run_id": output_root.name,
        "plan": candidate_a_execution_plan(config), "actual_text_values_stored": False,
    })
    ...
    try:
        initial_evaluation = evaluate_language_model(
            trainer.model, evaluation_loader, device=trainer.device, use_amp=trainer.amp_enabled,
        )
        result = trainer.train(target_steps=config.max_steps, metric_observer=observe_and_consume)
        ...
        if time.perf_counter() - started >= int(config.wall_clock_budget["hard_stop_seconds"]):
            raise TrainingError("FULL_PRETRAINING_WALL_CLOCK_LIMIT", "Wall-clock hard stop reached.")
        ...
        summary = {
            "schema_version": "1.0", "status": "completed", "global_step": trainer.state.global_step,
            "scheduled_tokens": trainer.state.global_step * config.tokens_per_optimizer_step,
            ...
            "best_reference": {"checkpoint": final_path.name, "basis": "final_full_internal_evaluation"},
            "actual_text_values_stored": False,
            "approval_consumed": approval_consumer.consumed,
        }
        resolved = config.to_dict()
        _write_json(output_root / "full-config-resolved.json", resolved)
        _write_json(output_root / "full-environment-manifest.json", collect_environment(repository_root()))
        ...
        _write_json(output_root / "full-checkpoint-checksum-manifest.json", checkpoint_manifest)
        _write_json(output_root / "full-resource-report.json", {
            "schema_version": "1.0", "elapsed_seconds": summary["elapsed_seconds"],
            "output_bytes": _directory_bytes(output_root), "remaining_disk_bytes": shutil.disk_usage(output_root).free,
        })
        _write_json(output_root / "full-run-summary.json", summary)
        _write_json(output_root / "full-completion-report.json", {
            "schema_version": "1.0", "status": "completed", "global_step": trainer.state.global_step,
            "checkpoint_count": 2, "automatic_extension": False,
        })
        _write_json(output_root / "full-execution-manifest.json", {
            "schema_version": "1.0", "status": "completed", "run_id": output_root.name,
            "plan": candidate_a_execution_plan(config), "actual_text_values_stored": False,
        }, replace=True)
        return summary
    except Exception as exc:
        _write_json(output_root / "full-failure-report.json", {
            "schema_version": "1.0", "status": "failed", "error_type": type(exc).__name__,
            "error_code": getattr(exc, "code", None), "global_step": trainer.state.global_step,
            "automatic_retry": False, "actual_text_values_stored": False,
